"""
#[1] Создайть базовый класс `Animal`, который будет содержать общие атрибуты (`name`, `age`) [1.1] и методы
(`make_sound()` [1.2], `eat() [1.3]`) для всех животных.
#[2] Реализовать наследование, создав подклассы `Bird` [2.1], `Mammal` [2.2], и `Reptile` [2.3], которые наследуют
от класса `Animal`. Добавить специфические атрибуты [2.4 - 2.6] и переопределить методы [2.7 - 2.9], если требуется
(например, различный звук для `make_sound()`).
#[3] Продемонстрировать полиморфизм: создайть функцию `animal_sound(animals)` [3.1], которая принимает список
животных [3.2] и вызывает метод `make_sound()` для каждого животного [3.3].
#[4] Используйть композицию для создания класса `Zoo`, который будет содержать информацию о животных [4.1],
и сотрудниках [4.2]. Должны быть методы для добавления животных [4.3] и сотрудников [4.4] в зоопарк.
#[5] Создайть классы для сотрудников, например, `ZooKeeper` [5.1], `Veterinarian` [5.2], которые могут иметь
